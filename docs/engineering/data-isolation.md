...
- **An exempt table scopes nothing, so the application is the only thing scoping access to it — and
  ...
  | `session_tokens` | `INSERT` when a session is established — **no caller yet**: the reading half above ships ahead of the minting half, so this row describes the rule the first writer inherits | the session it hangs off, written in the same save. The composite foreign key `(session_id, user_id) → sessions(id, user_id)` means a row whose owner disagreed with its session's is unstorable, so this one is scoped by the schema and not only by the caller |
