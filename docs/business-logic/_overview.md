...
| **Wrapped key** | An account key **wrapped under** a factor's key-encryption key, in the ciphertext envelope above — exactly 61 bytes over a 32-byte key, a width rather than a cap, and the entity's own rule rather than the format's. The only one of these four that ever reaches the server. |
