...
- [Account Keys](account-keys.md) — the one pair of keys an account owns, the key-encryption key
  ...
  carries the manifest of factor public keys, which stands in the schema with nothing writing it,
  and it names the one gap this slice leaves: the browser still produces the arrangement the routes
  stopped accepting.
- [Ciphertext Envelope](ciphertext-envelope.md) — the AEAD framing both consumers share, the
  ...
  product defines and nothing yet stores.
- [Key Rotation](key-rotation.md) — the remedy for a compromise of the keys themselves, why the
  ...
  is what the per-factor key pair exists for. Schema and domain behaviour only today; no route
  reaches it, and the begin currently does without a guarantee it used to hold.
