...
| **Registration** | The one request that brings an account into existence, and the **only** one: a caller the identity provider vouched for completes a WebAuthn ceremony, and the account, its budget, its three credentials, the passkey's key material, ten recovery-code hashes, eleven factor rows and the session it signs them in on all land in **one save, or none**. See [registration.md](registration.md). |
...
| **Recovery factor** | One secret an account holder possesses that can get them back into the account — **not** the same as one credential. A registered passkey is one factor; a set of recovery codes is **ten**, because each code is a secret of its own and a person redeems whichever one they still have. A federated credential is neither: it returns claims rather than a secret. Each factor **owns an ECDH key pair and carries its own encapsulated copy of the account's keys**, both written in the same save as the credential by the only three paths that create one. A passkey opens its own copy in the browser, on a sign-in and again from the Account keys section of `/app/settings`; nothing redeems a code in a browser yet. See [Account Keys](account-keys.md). |
...
| **Index key** | 32 random bytes an account owns, drawn independently of the content key, that a blind index over a name **is** computed under — the browser computes one today, and `accounts.name_key`, `payees.name_key`, `category_groups.name_key` and `categories.name_key` are the four columns that store one. That set is closed: every name column the requirement blind-indexes now has its column, and no description column ever gets one. Never transmitted. One per account, and the reason is stronger than the content key's: two index keys produce two index values for one name, so the uniqueness constraint stops colliding while appearing to work. |
...
| **Key-encryption key** | 32 bytes a recovery factor derives — from an authenticator's PRF output, or from a recovery code — and wraps that factor's own ECDH private key under. Imported as a **non-extractable** `AES-GCM` key, never transmitted, and never readable back out of the browser's key store. That is a claim about the imported key; the bytes it was derived from exist for the length of the derivation and are zero-filled where it consumes them — see [Account Keys](account-keys.md). |
| **Ciphertext envelope** | The AEAD framing everything this product encrypts is carried in: `version(1) ‖ nonce(12) ‖ ciphertext ‖ tag(16)`, unpadded base64url on the wire, with `0x01` — AES-256-GCM, 96-bit nonce, 128-bit tag — its only version. Twenty-nine bytes is its **floor** and not a width, because an empty plaintext is legal and **produces** exactly that — *produces*, because a length statement takes none of the three verbs. The binding a ciphertext carries is **not inside it**: associated data is rebuilt from wherever the ciphertext was found, which is what makes one moved elsewhere fail to authenticate — and what makes a changed grammar byte unopen everything already sealed. Three consumers, two grammars, one framing — see [Ciphertext Envelope](ciphertext-envelope.md). |
| **Encapsulated value** | The product's **second** framing: `version(1) ‖ ephemeral public key(65) ‖ nonce(12) ‖ ciphertext ‖ tag(16)`, floor 94, with `0x01` — ECDH over NIST P-256, then HKDF-SHA-256, then the same AES-256-GCM. A value **encapsulated to** a public key, where the envelope above wraps one **under** a key whoever opens it already holds. **Three verbs are normative and never interchanged**: data is *sealed under* a key, a key is *wrapped under* a key, and a value is *encapsulated to* a public key — the third is RFC 9180's word for the ECDH step, taken precisely because "sealed *to*" would have stood one preposition from "sealed *under*". The **encapsulation framing** is the layout; an encapsulated value is bytes in it. **Both framings lead with `0x01` and nothing in the bytes says which is which** — the column they were read from is the only discriminator. **Two columns store one**: `wrapped_account_keys.encapsulated_account_keys`, the generation in force, and `key_rotation_seals.encapsulated_account_keys`, the next one staged beside it. See [Ciphertext Envelope](ciphertext-envelope.md). |
| **Narrative field** | Free text a person typed into their ledger, and the second consumer of the envelope above. Eight `table.column` pairs across six entities carry one. On the server it is a **value type** — the only thing a narrative column accepts — with no constructor, factory or conversion taking a `string`, so writing plaintext into one does not compile. That absent member, and not any test, is what holds "no narrative value is ever server-readable". **All eight now exist as columns, and nothing is left to seal.** Five are names: `budgets.name` — `bytea`, nullable, reached by no route, so every row is NULL and it is the one name column with no blind index — and `accounts.name`, `payees.name`, `category_groups.name` and `categories.name`, each `NOT NULL` and each carrying a blind index beside it. Three are descriptions, all nullable and none indexed, ever: `category_groups.description` (the first sealed **free-text** column in the product), `categories.description` and `transactions.description`. Every one but `budgets.name` is reached by a route that accepts the sealed shape, and the screens that write one — `/app/accounts`, the transaction form and both halves of `/app/categories` — seal and open it in the browser. See [Ciphertext Envelope](ciphertext-envelope.md), [Accounts](accounts.md), [Payees](payees.md), [Categories and Category Groups](categories.md) and [Transactions](transactions.md). |
| **Narrative field cap** | How large a sealed narrative field may be: **1024 bytes** for the five name columns, **2560** for the three description columns. Two numbers over field *classes*, never eight over fields. They bound the **envelope** and not the text — the only length this side can measure — so the plaintext underneath is 29 bytes shorter and the server never sees a character. Not to be read, or "corrected", as a character limit. Every column of both classes is live, so a helper reaching for the wrong constant is refusing values one column is meant to accept or widening another nobody asked to widen — quiet in both directions. |
...
| **Factor key pair** | The ECDH P-256 pair one recovery factor owns. Its **private half** is **wrapped under** that factor's key-encryption key, in the ciphertext envelope above — exactly **167 bytes** over a 138-byte PKCS#8 key, a width rather than a cap, and the entity's own rule rather than the format's. Its **public half** is stored in one place and never per row: the account's **factor manifest**, one authenticated blob naming the whole set, which every path that moves that set writes. See [ADR 0025](../decisions/0025-give-every-recovery-factor-an-ecdh-key-pair.md). |
| **Encapsulated account keys** | The account's content key and index key as **one 64-byte plaintext, content key first**, **encapsulated to** a factor's public half — exactly **158 bytes**, in the encapsulation framing below. The order of the two halves is a contract between clients; the server has never held the private half and can never check it. |
| **Factor identifier** | The client-minted `factor_id` of a `wrapped_account_keys` row, and the value the wrapped private key's associated data binds it to, so a copy moved to another factor fails to authenticate. Deliberately **not** the credential id — see [ADR 0018](../decisions/0018-give-the-wrapped-account-keys-a-policed-table-and-their-own-factor-identifier.md). |
| **Submission** | One code's share of a request that issues a set: that code's verifier, its own client-minted factor identifier, that code's wrapped private key and the account's two keys encapsulated to the matching public half. `POST /api/me/recovery-codes` and `POST /api/registration` each carry ten of them on a member spelled `codes`, never ten bare verifiers — the shape follows from a set being ten factors rather than one, and the shared name is one wire contract for both write paths. See [recovery-codes.md](recovery-codes.md). |
...
| **Full session** | The kind of session a credential the holder actually possesses opens: a passkey, held by their authenticator, or a set of recovery codes, which they wrote down. The key custody each carries reaches every account there is — registration is the only way an account exists, and it files a key pair and an encapsulated copy of the account's keys for the passkey and for every one of its ten codes — and on the passkey half the browser **opens** it, deriving a key-encryption key from the assertion, holding the account's two keys for the visit, and putting what opened through the account's own manifest before it says so — the manifest must be present, must open under the content key, must not name a generation this device has already watched the account pass, and must name exactly the factors the response served. Nothing redeems a code in a browser yet, so on that half possession is still the whole of the reason. All four establishing paths give 14 days. |
...
| **Transaction** | A money movement against an Account on a calendar date. Its **description is a sealed narrative envelope**, nullable and never indexed, and `transactions` is the only sealed table with **no name column at all** — so no blind index, no unique name rule and no `IndexedName` reach it. See [transactions.md](transactions.md). |
...
| **Category** | Budget-owned transaction classification belonging to exactly one Category Group, e.g. “Groceries.” Its **name is a sealed narrative envelope with a blind index beside it** — the fourth and last blind index in the product — and its **description is a sealed envelope with nothing beside it**. Uniqueness survives the sealing, over `name_key` and still budget-wide across every group; what did not survive is every server-side rule about either text, the `case_insensitive` collation included, which left by force because `bytea` is not collatable. Both levels of [categories.md](categories.md) now read the same way. |
...
- [Account Keys](account-keys.md) — the one pair of keys an account owns, the key-encryption key
  each recovery factor derives, the ECDH key pair that key-encryption key now unwraps, the blind
  index and the normalization it is taken over, and the cross-client cryptographic contract. It also
  carries the manifest of factor public keys, which registration files and three server-side paths
  promote, the four ordered refusals the browser puts an opened factor through before it publishes
  the account's keys — the only check anywhere that catches those two keys encapsulated the wrong way
  round — and the per-device record of how far an account has rotated that one of the four compares
  against.
- [Ciphertext Envelope](ciphertext-envelope.md) — the AEAD framing both consumers share, the
  ...
  product defines and two columns store.
...
- [Transactions](transactions.md) — transaction rules, optional payee and category context, partial
  edit, delete, and the one sealed table that has no name column to index.
...
- [Categories and Category Groups](categories.md) — hierarchy, uniqueness, ordering, movement, and
  delete guards — and the chapter that owns four of the eight narrative columns, more than any other.
