...
| **Recovery factor** | Anything an account holder possesses that can get them back into the account. Today: a registered passkey, and a recovery code. A federated credential is not one — it returns claims rather than a secret. The unwrapping of account keys these factors are designed to carry is not built. |
