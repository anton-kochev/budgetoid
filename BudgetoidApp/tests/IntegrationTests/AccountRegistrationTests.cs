...
public sealed class AccountRegistrationTests
{
    ...
    private const string OptionsPath = "/api/registration/options";
    ...
    /// <c>codes</c>.
    /// </remarks>
    private const string CodesMember = "codes";
    ...
    /// <remarks>
    /// <para>
    /// It does <b>not</b> assert on the status. Several tests here read the response as their subject, and
    /// a helper that threw on a refusal would turn a red about the route into a red about the fixture.
    /// </para>
    /// <para>
        ...
        WrappedKeyFixture? passkeyKeys = null)
    {
        ...
        AttestationResult attestation = device.Register(
            ...
            // hands its authenticator. Deriving it here instead would make every device in this file
            // agree with the finish leg's own derivation whatever the options leg said — see
            // Registration_ThenAssertion_PresentsTheHandleTheDeviceWasGiven, which is the test that
            // reads it back.
            userHandle: options.UserHandle);

        return new RegisteredAccount(
            await PostRegistrationAsync(client, attestation, keys, card),
            options.Challenge,
    ...
    /// </remarks>
    private sealed record IssuedRegistrationOptions(byte[] Challenge, byte[] UserHandle);
    ...
    /// </remarks>
    private static async Task<IssuedRegistrationOptions> BeginRegistrationCeremonyAsync(HttpClient client)
    {
    ...
    /// <remarks>
    /// <para>
    /// <b>The card arrives on a member called <c>codes</c>, and the name is a rule rather than a
    /// preference.</b> <see cref="TestSupport.UnwrappedKeyMaterialVocabulary" /> refuses the token
    /// <c>recovery_code</c> unless <c>hash</c> follows it, deliberately and on this one word, so a
    /// request record declaring <c>RecoveryCodes</c> is reported by
    /// <see cref="KeyMaterialSecrecyTests.RequestSurface_CarriesNoMemberThatCouldHoldUnwrappedKeyMaterial" />.
    /// The already-shipped sibling route spells the identical payload <c>codes</c> for exactly that
    /// reason, and two write paths accepting one set of recovery codes have no business disagreeing about
    /// what the member is called. The other way out — a <c>JsonPropertyName</c> attribute over a
    /// differently named property — is the evasion that vocabulary's own remarks warn about.
    /// </para>
    /// </remarks>
    ...
    /// Two of the refusals below are about an <em>absent</em> member — no <c>clientExtensionResults</c>
    ...
    /// </remarks>
    private static async Task<HttpResponseMessage> PostAmendedRegistrationAsync(
        HttpClient client,
        SyntheticAuthenticator device,
    ...
    /// </remarks>
    private sealed record CodeSubmission(string Verifier, WrappedKeyFixture Keys)
    {
    ...
    private static IReadOnlyList<CodeSubmission> CardOf(IReadOnlyList<string> verifiers) =>
        [.. verifiers.Select(verifier => new CodeSubmission(verifier, WrappedKeyFixture.Mint()))];
