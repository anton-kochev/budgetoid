    ...
    : ICommandHandler<UpdateCategoryCommand>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // neighbour looks its row up first and judges the body second; this one judges first, the way
        // UpdateCategoryGroupHandler does. The order is a real choice and neither leaks existence - a
        // caller with a malformed body learns nothing about the row either way, because it is refused
        // before anything is read. What decides it is that this is a shape check on the request needing
        // nothing from the database, and that the answer a caller can act on is the one naming the member
        // they sent: run below the lookup, a malformed body against an unknown id answers 404, which
        // tells the caller to go looking for a row when the fault is in their own request.
        //
        ...
        // CreateCategoryHandler gives for its own: the three are produced by one piece of client code, so
        // a caller that got two of them wrong would otherwise learn about the second only after fixing
        // the first and sending everything again. No identifier among them - the route parameter stays
        // {id:guid} and needs no canonical check, because a client re-sealing an update binds the row's
        // EXISTING id. UpdateCategoryCommand's remarks carry that argument.
        ...
        // CK_categories_description_length as a 23514 nothing translates.
        //
        ...
        // The lookup runs through the budget query filter, so a category belonging to another budget is
        ...
        // Suppressed rather than re-checked, for the reason CreateCategoryHandler gives: an empty
        ...
        // split, where a create answers 409. CategoryRepository argues it at AddAsync and at
        // DuplicateNameValidationException, which the two verbs share.
