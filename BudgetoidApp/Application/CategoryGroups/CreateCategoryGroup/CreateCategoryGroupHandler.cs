...
/// <summary>
/// Writes the category group a client minted, sealed and indexed, and appends it to the person's own
/// ordering.
/// </summary>
/// <remarks>
/// <b>Four members arrive and this server can read exactly one of them.</b> The identifier is checked for
/// a spelling it can reproduce; the other three are opaque and judged for shape alone. The position is
/// the one value the server still computes, because it is the one value it can still read.
/// </remarks>
    ...
    : ICommandHandler<CreateCategoryGroupCommand, CategoryGroupDto>
{
    ...
    {
        // The members this server cannot read, judged for shape and nothing else, above the entity
        ...
        // factories throw ArgumentException rather than the exception that becomes a 400.
        //
        // EVERY MEMBER IS ATTEMPTED AND EVERY FAILURE IS REPORTED - the idiom CreatePayeeHandler and
        // CreateAccountHandler keep for their own members. They arrive together, are produced by one
        // piece of client code and are opaque to this side in the same way, so a client that got two of
        // them wrong would otherwise learn about the second only after fixing the first and sending
        // everything again. The description is the member most at risk of losing that property, because
        // it sits behind a branch: judged inside an early return, or after the throw below, it would
        // never be reported alongside the others.
        ...
        if (!CanonicalIdentifier.TryParse(command.Id, out Guid id))
        {
            ...
            [
                "The category group id must be a uuid in the lower-case 36-character hyphenated form "
        ...
        // CK_category_groups_description_length, reaching the caller as a 23514 nothing translates rather
        // than as the 400 it should have been.
        //
        ...
        // group with no note reaches SealedOrAbsent as null and comes back null. That member is the one
        ...
        CategoryGroup categoryGroup = CategoryGroup.Create(
            ...
            NarrativeField.SealedOrAbsent(
                descriptionEnvelope, NarrativeFieldLimits.DescriptionBytes),
            position,
            timeProvider.GetUtcNow().UtcDateTime);

        await repository.AddAsync(categoryGroup, cancellationToken);

        // From the entity this handler just wrote, never rebuilt member by member here: the 201 body and
        // the body GET /api/category-groups/{id} answers have to encode identically, and one factory is
        // what makes that a fact rather than two call sites that happen to agree today.
        return CategoryGroupDto.FromCategoryGroup(categoryGroup);
    }
}
