...
/// <summary>
/// The budget every request operates inside — the unit of tenancy. Resolved eagerly, while the request
/// authenticates, because the global query filters read it synchronously while a query is being built.
/// </summary>
public interface IBudgetContext
{
    /// <summary>
    /// The ambient budget, or <see langword="null"/> when the current request has not resolved one — an
    /// anonymous route reaches no tenant, an ended session is refused one on purpose, and infrastructure
    /// scopes such as health checks never have a request principal at all. This is the accessor for
    /// callers that must
