...
public static class MappedSchema
{
    ...
    /// <see cref="MappedColumn.ClrType" /> is <b>the ordinary case rather than the edge one</b> — 63 of
    /// the 93 mapped columns answer neither accessor, against 29 carrying a converter and exactly one
    ...
    /// in this schema, and the file should not imply otherwise.</b> Every one of the 93 mapped columns
    ...
    /// narrative, so a walk returning those eight and dropping the other eighty-five answers every
    ...
    /// remaining ninety-one. A walk handing <c>users.email</c> some other table's constraints agrees on
